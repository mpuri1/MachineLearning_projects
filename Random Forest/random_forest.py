...
class RandomForest(object):
    ...
    def _bootstrapping(self, XX, n):
        ...
        for i in range(n):
            attr = XX[np.random.randint(0,n-1)]
            labels.append(attr[-1])
            samples.append(attr[:-1]) 
    ...
    def voting(self, X):
        ...
        for record in X:
            ...
            if len(counts) == 0:
                ...
                idx = self.bootstraps_datasets[0].index(record)
                y = np.append(y, self.bootstraps_labels[0][idx])
                print ("not an out-of-bag model"),idx,y
            else:
...
def main():
    ...
    numerical_cols = set([i for i in range(0,42)]) # indices of numeric attributes (columns)
